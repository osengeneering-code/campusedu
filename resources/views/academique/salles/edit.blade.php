        ...
        <div class="card-body">
            <form action="{{ route('academique.salles.update', $salle) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('type_salle') is-invalid @enderror" id="type_salle" name="type_salle" value="{{ old('type_salle', $salle->type_salle) }}" placeholder="Ex: Salle de cours, Laboratoire" />
                    @error('type_salle')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
